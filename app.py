...
async def synthesize_treatment_advice_and_citations(treatment_matches: List[Dict], user_query: str) -> (str, List[str]):
    ...
    Use GPT-2 (OpenAI client2) to:
      ...
      2. Provide source citations if possible. If none, say 'Data gotten from our training data.'
    """
    try:
        ...
        if not qa_snippets:
            ...
            citations = ["Data gotten from our training data."]
        ...
        prompt = (
            ...
            "(e.g., WHO, CDC, peer-reviewed journals). If you cannot find citations, end with 'Data gotten from our training data.'\n\n"
            f"User's question: {user_query}\n\n"
            "Relevant Q&A excerpts:\n\n"
            + "\n\n---\n\n".join(qa_snippets) +
            "\n\nPlease provide:\n1. Treatment recommendations (concise).\n2. Source citations or fallback statement."
        )
        response = await client2.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role": "system", "content": "You are a helpful medical assistant providing treatment guidance and citations."},
                      {"role": "user", "content": prompt}],
            temperature=0.3,
            max_tokens=400
        )
        result_text = response.choices[0].message.content.strip()
        # Attempt to split recommendations and citations if formatted
        ...
        return ("I recommend consulting with a healthcare provider for personalized treatment advice.",
                ["Data gotten from our training data."])
...
async def generate_info_request_response(thread_id: str, question: str, client=None) -> TriageResponse:
    ...
    Otherwise, fallback to a direct GPT answer.
    """
    # 1. Query treatment index with the question as a plain query (no symptoms list)
    treatment_index = client_manager.get_treatment_index()
    citations: List[str] = []
    answer_text = ""
    matches = []
    if treatment_index:
        try:
            ...
            response = treatment_index.query(
                vector=query_embedding[0], top_k=5, include_metadata=True
            )
            matches = response.get("matches", [])
            ...
            matches = []
    # 2. If we have matches, send to GPT2 for synthesis & citations
    if matches:
        advice, citations = await synthesize_treatment_advice_and_citations(matches, question)
        answer_text = advice
    else:
        # No matches: fallback to direct GPT answer using client2
        try:
            client2 = await client_manager.get_openai_client_2()
            prompt = (
                ...
                "If you have no citations, say 'Data gotten from our training data.'\n\n"
                f"Question: {question}"
            )
            response = await client2.chat.completions.create(
                model="gpt-4o-mini",
                messages=[{"role": "system", "content": "Answer general medical questions with citations if possible."},
                          {"role": "user", "content": prompt}],
                temperature=0.3,
                max_tokens=300
            )
            full_answer = response.choices[0].message.content.strip()
            # Try to extract citations if labeled
...
async def generate_conversational_response(context: Dict, is_emergency: bool, thread_id: str, client=None) -> TriageResponse:
    """Generate conversational medical response that feels like talking to a real nurse, with citations"""
    try:
        ...
        response_data = {
            ...
            "citations": [],  # citations only apply in treatment section above
...
async def triage(request: TriageRequest):
    ...
    try:
        ...
        if intent_label == "GREETING":
            ...
            # Use the same thread but delegate to info handler
...
async def test_treatment():
    ...
    try:
        ...
        if treatment_matches:
            ...
            advice, citations = "No treatment matches found", ["Data gotten from our training data."]
        return {
            "status": "success",
...
async def health_check():
    ...
    try:
        ...
        return HealthResponse(
            ...
            timestamp=datetime.utcnow().isoformat(),
            version="1.1.0",
            services={"openai": openai_status, "pinecone": pinecone_status, "treatment_index": treatment_status}
        )
    except Exception as e:
        logger.error(f"Health check error: {e}")
        return HealthResponse(status="unhealthy", timestamp=datetime.utcnow().isoformat(), version="1.1.0", services={"error": str(e)})
